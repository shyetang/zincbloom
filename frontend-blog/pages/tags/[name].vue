            ...
            >
              ...
              <div class="flex items-center gap-1">
                <UIcon
                  ...
                  class="w-4 h-4"
                />
                <span>{{ formatDate(tag.created_at) }} 创建</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          >
            ...
            <UIcon
              ...
              class="w-4 h-4 text-slate-400"
            />
            <span style="color: #10b981; font-weight: 500">#{{ tag.name }}</span>
          </div>
        </div>
      </nav>
          ...
          >
            ...
            >
              <button
                ...
                :key="relatedTag.id"
                class="inline-flex items-center gap-2 px-4 py-2 rounded-full transition-all hover:transform hover:scale-105"
                style="
                                    background: linear-gradient(
                                        45deg,
                                        rgba(139, 92, 246, 0.1),
              ...
              >
                ...
                <span class="text-xs opacity-70">({{ relatedTag.post_count || 0 }})</span>
              </button>
            </div>
          </div>
        </div>
...
import type { PostQueryParams, Tag } from "~/types";

// 获取路由参数
const route = useRoute();
const tagName = decodeURIComponent(route.params.name as string);
...
const {
  ...
  error,
  refresh,
} = await useLazyAsyncData(`tag-${tagName}`, async () => {
  const response = await api.getTag(tagName);
  if (response.success) {
...
const {
  ...
  `tag-posts-${tagName}`,
  async () => {
    const response = await api.getPostsByTag(tagName, queryParams.value);
    if (response.success) {
      return response.data;
    }
    return {
...
// 获取相关标签（简化版本，实际应该从API获取）
const { data: allTags } = await useLazyAsyncData(
  "all-tags-for-related",
  async () => {
...
const goToTag = (name: string) => {
  navigateTo(`/tags/${encodeURIComponent(name)}`);
};
  ...
  if (tag.value) {
    useHead({
      ...
      meta: [
        {
          name: "description",
          content: `浏览包含 #${tag.value.name} 标签的所有文章`,
        },
        { property: "og:title", content: `#${tag.value.name} - 标签` },
        {
